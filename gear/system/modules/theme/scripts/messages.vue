<template>
    <li v-bind:class="{ bell: true, active: listMessages.length, open: open }" ref="messageList">
        ...
        <div v-bind:class="{ list: true, open: open }">
            <h4>
                {{ 'Messages' | lang }}
                <span v-text="listMessages.length"></span>
            </h4>
            <ul v-if="listMessages.length">
                <li v-for="(item, index) in listMessages" :class="item.type">
                    <div v-html="icons[item.type]"></div>
                    {{ item.message | lang }}
